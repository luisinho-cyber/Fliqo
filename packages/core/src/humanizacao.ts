...
export const RITMO_PADRAO: RitmoClinica = {
  minRespostaMs: 4_000,
  maxRespostaMs: 45_000,
  caracteresPorSegundo: 4,
};
...
export function planejarEnvio(
  ...
  const aguardarAntesMs = clamp(
    Math.round(leitura + pensar),
    ritmo.minRespostaMs,
    ritmo.maxRespostaMs,
  );
  ...
  const baloes = quebrarEmBaloes(resposta).map((texto) => ({
    texto,
    digitandoMs: clamp(
      Math.round((texto.length / ritmo.caracteresPorSegundo) * 1_000 * (0.85 + rnd() * 0.3)),
      1_200,
      12_000,
    ),
  }));
...
export function quebrarEmBaloes(texto: string, maxBaloes = 3, alvo = 160): string[] {
  ...
  for (const f of frases) {
    ...
    if (
      ultimo !== undefined &&
      (ultimo.length + f.length + 1 <= alvo || baloes.length === maxBaloes)
    ) {
