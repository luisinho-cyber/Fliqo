...
describe('precificação (markup divisor)', () => {
  it('caso canônico: custo R$ 50 -> R$ 80,75, não R$ 69,04', () => {
    const preco = precoPorMarkupDivisor(5_000, { impostoBp: 808, taxaCartaoBp: 1_000, comissaoBp: 0, lucroBp: 2_000 });
    expect(preco).toBe(8_075);
  });
  it('percentuais >= 100% são rejeitados', () => {
    expect(() => precoPorMarkupDivisor(5_000, { impostoBp: 5_000, taxaCartaoBp: 3_000, comissaoBp: 1_000, lucroBp: 1_000 })).toThrow();
  });
  it('lucro que sobra bate com o lucro pedido', () => {
    const preco = precoPorMarkupDivisor(5_000, { impostoBp: 808, taxaCartaoBp: 1_000, comissaoBp: 0, lucroBp: 2_000 });
    const m = margemDeContribuicao(preco, 5_000, { impostoBp: 808, taxaCartaoBp: 1_000, comissaoBp: 0 });
    expect(Math.abs(m.bp - 2_000)).toBeLessThanOrEqual(1);
  });
});

describe('financeiro', () => {
  const atendimento = { procedimento: 'Botox', preco: 150_000, custoInsumos: 40_000, comissaoBp: 1_000, data: '2026-01-31' };

  it('crédito 3x: receita nas datas em que o dinheiro cai, taxa e comissão separadas', () => {
    const l = lancamentosDoAtendimento(atendimento, { nome: 'Crédito 3x', taxaBp: 350, diasParaReceber: 30, parcelas: 3 });
    ...
    expect(l.filter((x) => x.categoria === 'taxa_pagamento').reduce((s, x) => s + x.valor, 0)).toBe(5_250);
  ...
  it('parcela mensal respeita fim de mês (31/01 -> 28/02)', () => {
    const l = lancamentosDoAtendimento({ ...atendimento, data: '2026-01-31' }, { nome: 'Crédito 2x', taxaBp: 0, diasParaReceber: 0, parcelas: 2 });
    expect(l.filter((x) => x.tipo === 'receita').map((r) => r.vencimento)).toEqual(['2026-01-31', '2026-02-28']);
  });

  it('Pix à vista: uma receita, sem taxa', () => {
    const l = lancamentosDoAtendimento({ ...atendimento, comissaoBp: 0, custoInsumos: 0 }, { nome: 'Pix', taxaBp: 0, diasParaReceber: 0, parcelas: 1 });
    expect(l).toEqual([{ tipo: 'receita', status: 'previsto', categoria: 'procedimento', descricao: 'Botox — Pix', valor: 150_000, vencimento: '2026-01-31' }]);
  });
  ...
  it('custo das faltas', () => {
    expect(custoDasFaltas([{ preco: 20_000 }, { preco: 35_000 }])).toEqual({ quantidade: 2, valor: 55_000 });
  });
});
...
describe('agenda', () => {
  ...
  it('respeita antecedência mínima', () => {
    const livres = horariosLivres({ expediente: [{ inicio: d(9), fim: d(12) }], ocupados: [], duracaoMin: 60, passoMin: 30, agora: d(9, 10), antecedenciaMinimaMin: 60 });
...
describe('lista de espera e confirmação', () => {
  ...
  it('vaga daqui a 40 min: não oferta', () => {
    expect(planejarOferta(cfg, new Date('2026-11-10T12:40:00Z'), agora)).toEqual({ ofertar: false, motivo: 'em_cima_da_hora' });
  });

  it('modo sequencial oferta para 1 por vez', () => {
    const p = planejarOferta({ ...cfg, modo: 'sequencial' }, new Date('2026-11-10T18:00:00Z'), agora);
    expect(p.ofertar && p.quantos).toBe(1);
  });

  it('botões do template são lidos sem IA; texto livre vai para a IA', () => {
    expect(efeitoDaResposta(interpretarResposta({ payloadBotao: PAYLOAD_BOTOES.CONFIRMAR }))).toEqual({ acao: 'marcar_confirmado' });
    expect(efeitoDaResposta(interpretarResposta({ payloadBotao: PAYLOAD_BOTOES.CANCELAR })).acao).toBe('liberar_horario_e_ofertar');
    expect(efeitoDaResposta(interpretarResposta({ texto: 'vou atrasar uns 10 min' }))).toEqual({ acao: 'encaminhar_para_ia' });
  });
});
