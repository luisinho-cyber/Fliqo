...
export async function seed(owner: pg.Pool): Promise<Scenario> {
  const one = async (sql: string, p: unknown[] = []): Promise<string> => {
    const { rows } = await owner.query<{ id: string }>(sql, p);
    const linha = rows[0];
    if (!linha) throw new Error(`insert do cenário não devolveu id: ${sql}`);
    return linha.id;
  };
...
/**
 * Cria o schema da fila no banco de teste, com a conexão de dono — o mesmo que o
 * db:migrate faz em produção. Quem enfileira nos testes é o papel da aplicação.
 */
export async function prepararFilaDeTeste(): Promise<void> {
  const { prepararFila } = await import('../scripts/fila.mjs');
  await prepararFila(withDb(ADMIN_URL, TEST_DB));
}
